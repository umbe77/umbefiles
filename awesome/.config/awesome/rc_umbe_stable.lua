...
mytextclock:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then w_calendar.toggle() end
end)
...
local w_systray = wibox.widget.systray(true)
w_systray:set_base_size(18)
-- w_systray["base_size"] = 20
w_systray["forced_height"] = 18
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mywibox:setup{
        ...
        { -- Right widgets
            ...
            --        arc_thickness = 1
            --    }),
            openweather_widget({margin_left = 5, margin_right = 5}),
            mytextclock,
...
awful.key({altkey}, "space",
    ...
    awful.spawn.with_shell("$HOME/.scripts/dmenu_run_colors")
end, {description = "dmenu launcer", group = "launcher"}),
...
end, {description = "restore minimized", group = "client"}), awful.key({modkey},
                                                                       "d",
                                                                       function()
...
awful.rules.rules = {
    ...
    }, -- Floating clients.
    {
        rule_any = {
            ...
            class = {
                "Yad", "Arandr", "Blueman-manager", "Gpick", "Kruler",
                "MessageWin", -- kalarm.
                "Sxiv", "Tor Browser", -- Needs a fixed window size to avoid fingerprinting by screen size.
                "Wpa_gui", "Ulauncher", "veromix", "xtightvncviewer"
            },
...
client.connect_signal("property::floating", function(c)
    if c.floating and c.class ~= "umbe_sticky_terminal" then
...
require('awful.dbus')
assert(dbus)
assert(dbus.connect_signal("org.awesomewm.awful.umbe", function(data, echoString)
    naughty.notify{
        text = data.member
    }
    if data.member == "echo" then
        return "s", echoString
    end
    return "s", "No Method"
end))
--]]
-- dbus-send --dest=org.awesomewm.awful --type=method_call --print-reply / org.awesomewm.awful.umbe.echo string:"pippo"
